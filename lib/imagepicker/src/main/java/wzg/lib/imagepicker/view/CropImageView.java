package wzg.lib.imagepicker.view;
...
import wzg.lib.imagepicker.utils.Utils;
...
public class CropImageView extends AppCompatImageView
{
	...
	/**
	 ...
	 @param cropRound 是否按圆形区域保存图片
	 @return 裁剪后的Bitmap
	 */
	public Bitmap getCropBitmap(int expectWidth, int exceptHeight, boolean cropRound){
		...
		return makeCropBitmap(srcBitmap, mFocusRect, getImageMatrixRect(), expectWidth, exceptHeight, cropRound);
	}
	...
	/**
	 ...
	 @param cropRound 是否希望按圆形区域保存图片
	 @return 裁剪后的图片的Bitmap
	 */
	private Bitmap makeCropBitmap(Bitmap bitmap, RectF focusRect, RectF imageMatrixRect, int expectWidth,
			int exceptHeight, boolean cropRound){
		...
		try{
			...
			if(expectWidth!=width || exceptHeight!=height){
				...
				if(mStyle==CropImageView.Style.CIRCLE && cropRound){
					//如果是圆形，就将图片裁剪成圆的
	...
	/**
	 ...
	 @param cropRound 是否希望按矩形区域保存图片
	 */
	public void saveBitmapToFile(File folder, int expectWidth, int exceptHeight, boolean cropRound){
		if(mSaving) return;
		mSaving=true;
		final Bitmap croppedImage=getCropBitmap(expectWidth, exceptHeight, cropRound);
		Bitmap.CompressFormat format;
		File saveFile;
		if(mStyle==CropImageView.Style.CIRCLE && cropRound){
			format=Bitmap.CompressFormat.PNG;
			saveFile=Utils.createFile(folder, "CROP_", ".png");
		}else{
			format=Bitmap.CompressFormat.JPEG;
			saveFile=Utils.createFile(folder, "CROP_", ".jpg");
		}
		final Bitmap.CompressFormat finalOutputFormat=format;
