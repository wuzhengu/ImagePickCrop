...
import wzg.lib.imagepicker.data.ItemType;
...
import wzg.lib.imagepicker.view.SquareImageView;
import wzg.lib.imagepicker.view.SquareRelativeLayout;
	...
	private void bindMedia(MediaHolder mediaHolder, MediaFile mediaFile){
		...
		if(!TextUtils.isEmpty(imagePath)){
			...
			if(SelectionManager.getInstance().isImageSelect(imagePath)){
				...
				mediaHolder.mImageCheck.setImageDrawable(
						mContext.getResources().getDrawable(R.mipmap.icon_image_checked));
			}else{
				mediaHolder.mImageView.setColorFilter(null);
				mediaHolder.mImageCheck.setImageDrawable(
						mContext.getResources().getDrawable(R.mipmap.icon_image_check));
			}
			try{
